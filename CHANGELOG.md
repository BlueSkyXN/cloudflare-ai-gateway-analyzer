...
- FastAPI control plane at `/api/v1/*` (17 paths) replacing the Streamlit subprocess.
...
- 78 unit + integration tests (pytest + pytest-asyncio + httpx MockTransport).
...
- Analytics aggregations are now pushed down to SQLite (percentiles via `OFFSET` index lookup), avoiding Python-side full scans.
