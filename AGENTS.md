...
| `src/cf_aigw_analyzer/data/`        | SQLite schema v7, migrations, repositories, row models                                          | No              | Before any DB schema, index, or repository contract change                 |
...
- SQLite schema v7 keeps `log_events` as the single analytics fact table and `log_raw` as the sanitized JSON side table.
