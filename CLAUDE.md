...
`cloudflare-ai-gateway-analyzer` v0.3 是一个**本地优先的 Cloudflare AI Gateway 日志分析工具**，包含：

- Typer CLI（采集、查询、状态）
- FastAPI 控制面（`/api/v1/*`，OpenAPI 自动产出）
- React + Vite + ECharts 看板（默认暗色、仪表盘式信息密度）
- 单 SQLite 文件存储（scope 由 `(account_id, gateway_id, log_id)` 标识）

不是 gateway，不对外暴露代理服务；不引入消息队列、外部数据库或多租户。

## 关键设计约束（别破坏）

- **不持久化请求/响应正文**。`core.sanitizer.sanitize_log_metadata` 是底线，凡涉及 raw_json 的代码路径都得过它。
...
- **数据迁移**：v0.2 → v0.3 不支持自动迁移（user_version 跨度大，schema 也变了）。新升级走 `data/migrations.py` 的 handler 链。
...
- `analytics/` 与 `control/routes/analytics_route.py` 一一对应，前后端契约通过 `control/schemas/analytics.py` 锁定。
...
- 跑测试统一用 `PYTHONPATH=src python -m pytest -q`，CI 用 `python scripts/smoke_local.py` 一键跑全套。

## 常用命令

| 命令                                          | 用途                                                  |
| --------------------------------------------- | ----------------------------------------------------- |
| `PYTHONPATH=src python -m pytest -q`          | 78 个单元 + 集成测试，<5s                              |
| `python -m ruff check src tests scripts ...`  | Lint，零容忍                                          |
| `python -m ruff format --check src tests ...` | 格式校对                                              |
| `python cli.py serve`                         | 启动 FastAPI + 看板（端口 8765）                       |
| `python cli.py config show`                   | 看脱敏后的有效配置                                     |
| `python scripts/seed_sqlite.py --count 200`   | 灌入合成数据（不真打 Cloudflare）                      |
| `python scripts/check_api.py`                 | ASGI 内存调用所有 GET 路由，确保返回 200               |
| `python scripts/generate_openapi.py`          | 导出 `local/openapi.json`                              |
| `python scripts/smoke_local.py`               | ruff + pytest + openapi + check_api 一站式            |
